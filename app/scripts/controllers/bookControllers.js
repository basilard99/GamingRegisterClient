...
bookControllers.controller('addBookController', function defineController($http, locationService, publisherListService) {
    ...
    vm.addBook = function sendBookToApi() {
        ...
        $http.put(locationService.getBaseLocation() + '/api/books/' + encodeURIComponent(vm.book.title), vm.book)
             .then(function succeeded() { vm.addBookStatus = 'Book added successfully'; },
                   function failed(response) { vm.addBookStatus = 'Failed to add book: ' + response.status; });
    };
